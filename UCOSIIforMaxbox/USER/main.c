...
void beep_task(void *pdata)
{
	...
	while(1)
	{
		OSSemPend(sem_beep,0,&err);//该函数为等待一个信号量
		BEEP=1;
		delay_ms(60);
		BEEP=0;
		delay_ms(940);
	...
	while(1)
	{
		...
		{	
			...
			{			
				...
				lcd_draw_bline(lastpos[0],lastpos[1],tp_dev.x[0],tp_dev.y[0],2,BLACK);//画线
				OS_EXIT_CRITICAL();
				lastpos[0]=tp_dev.x[0];
				lastpos[1]=tp_dev.y[0];     
			}
...
{							 
	...
	while(1)
	{
		key=(u32)OSMboxPend(msg_key,10,&err);   //等待邮箱中的消息 收到不为空的消息指针，然后转换为消息内容
		switch(key)
		{
			case 1://控制DS1
				LED1=!LED1;
   		...
   		if(semmask||sem_beep->OSEventCnt)			//需要显示sem	//如果semmask和OSEventCnt都不为空则显示sem	
		{
			POINT_COLOR=BLUE;
...
{	
	...
	{
		key=KEY_Scan(0);   
		if(key)OSMboxPost(msg_key,(void*)key);//发送消息   首先判断key是否为空 不为空则把key值发送到邮箱//该函数规定pmsg指针一定不能为空
 		delay_ms(10);
	}
}
